...
LEMMA NPC == \A S: /\ NewProposals(S) \in (SUBSET [slot: FreeSlots(S), val: Values])
  ...
  <1> SUFFICES ASSUME NEW S
               PROVE  /\ NewProposals(S) \in (SUBSET [slot: FreeSlots(S), val: Values])
                      ...
                      /\ ~ \E t1 \in Bmax(S), t2 \in NewProposals(S): t1.slot = t2.slot
      OBVIOUS
  <1>1. \E T \in (SUBSET [slot: FreeSlots(S), val: Values]) : \A t1, t2 \in T: t1.slot = t2.slot => t1 = t2
        BY DEF FreeSlots
  <1>2. NewProposals(S) \in (SUBSET [slot: FreeSlots(S), val: Values])  BY <1>1 DEF NewProposals
  <1>3. \A t1, t2 \in NewProposals(S): t1.slot = t2.slot => t1 = t2  BY <1>1 DEF NewProposals
  <1>4. ~ \E t1 \in Bmax(S), t2 \in ([slot: FreeSlots(S), val: Values]) : t1.slot = t2.slot
...
LEMMA Phase2aVotedForInv == TypeOK => \A p \in Proposers: Phase2a(p) => \A a \in Acceptors, b \in Ballots, s \in Slots, v \in Values:
                              VotedForIn(a, b, s, v) <=> VotedForIn(a, b, s, v)'
BY DEF VotedForIn, Send, TypeOK, Messages, Phase2a, ProposeDecrees, Bmax, NewProposals, FreeSlots, PartialBmax
...
<1>1. Init => Inv  BY DEF Init, Inv, TypeOK, AccInv, MsgInv, VotedForIn
<1>2. Inv /\ [Next]_vars => Inv'
  ...
  <3>1. TypeOK'
    <4>1. ASSUME NEW p \in Proposers, Phase1a(p)  PROVE TypeOK'
      ...
      <5> QED BY <5>1, msgs' \ msgs \subseteq Messages DEF TypeOK, Send, Messages
    <4>2. ASSUME NEW p \in Proposers, Phase2a(p)  PROVE TypeOK'
      <5>1. PICK Q \in Quorums, S \in SUBSET {m \in msgs: (m.type = "1b") /\ (m.bal = pBal[p])}:
                ...
                /\ Send([type |-> "2a", from |-> p, bal |-> pBal[p],
                     decrees |-> ProposeDecrees(UNION {m.voted: m \in S})])  BY <4>2 DEF Phase2a
      <5>2. FreeSlots(UNION {m.voted: m \in S}) \subseteq Slots  BY <5>1 DEF FreeSlots, TypeOK, Messages
      <5>3. PartialBmax(UNION {m.voted: m \in S}) \subseteq [bal: Ballots, slot: Slots, val: Values]
            BY DEF PartialBmax, TypeOK, Messages
      <5>4. /\ NewProposals(UNION {m.voted: m \in S}) \subseteq [slot: Slots, val: Values]
            /\ Bmax(UNION {m.voted: m \in S}) \subseteq [slot: Slots, val: Values]  BY <5>3, <5>2, NPC DEF Bmax
      <5>5. ProposeDecrees(UNION {m.voted: m \in S}) \subseteq [slot: Slots, val: Values]  BY <5>4 DEF ProposeDecrees
      <5>6. \A m2 \in msgs' \ msgs: /\ m2.type = "2a" /\ m2.from = p /\ m2.bal = pBal[p]
                                    /\ m2.decrees = ProposeDecrees(UNION {m.voted: m \in S})
    ...
    <4>3. ASSUME NEW a \in Acceptors, Phase1b(a)  PROVE TypeOK'
      ...
      <5>2. \A m2 \in msgs' \ msgs: m2.type = "1b" /\ m2.from = a /\ m2.bal = m.bal /\ m2.voted = aVoted[a]
            BY <5>1 DEF Send
      <5>3. (msgs \in SUBSET Messages)'  BY <5>1, <5>2 DEF TypeOK, Messages, Send
      <5> QED BY <5>3, <4>3 DEF Phase1b, TypeOK
    <4>4. ASSUME NEW a \in Acceptors, Phase2b(a)  PROVE TypeOK'
      ...
      <5>2. \A m2 \in msgs' \ msgs: m2.type = "2b" /\ m2.from = a /\ m2.bal = m.bal /\ m2.decrees = m.decrees
            BY <5>1 DEF Send
      <5>3. (msgs \in SUBSET Messages)'  BY <5>1, <5>2 DEF TypeOK, Send, Messages
      <5> QED BY <5>3, <5>1, <4>4 DEF TypeOK, Messages
    <4> QED BY <2>1, <4>1, <4>2, <4>3, <4>4 DEF Next
  ...
  <3>2. AccInv'
    ...
    <4>4. ASSUME NEW a \in Acceptors, Phase2b(a)  PROVE AccInv'
      ...
      <5>2. CASE a2 # a
        ...
        <6>2. aVoted[a2] = aVoted'[a2] /\ aBal[a2] = aBal'[a2] BY <5>1, <5>2 DEF Send, TypeOK
        <6> QED BY <6>1, <6>2, <5>1, <5>2 DEF AccInv, TypeOK
      <5>3. CASE a2 = a
        ...
        <6>2. (\A r \in aVoted[a2]: VotedForIn(a2, r.bal, r.slot, r.val) /\ r.bal =< aBal[a2])'
          ...
          <7>1. VotedForIn(a2, r.bal, r.slot, r.val)'
            ...
            <8>2. CASE r \in aVoted'[a2] \ aVoted[a2]
              ...
              <9>2. r.bal = m.bal /\ \E d \in m.decrees: r.slot = d.slot /\ r.val = d.val  BY <5>1, <5>3, <3>1, <8>2 DEF TypeOK
              <9> QED BY <9>1, <9>2, <5>3 DEF Send, TypeOK, Messages, VotedForIn
            <8> QED BY <8>1, <8>2
          <7>2. (r.bal =< aBal[a2])'  BY <5>1, <5>3, <3>1 DEF AccInv, Send, TypeOK, Messages
          <7> QED BY <7>1, <7>2
        <6>3. (\A b \in Ballots, s \in Slots, v \in Values: VotedForIn(a2, b, s, v) => \E r \in aVoted[a2]: r.bal >= b /\ r.slot = s)'
          ...
          <7>1. CASE VotedForIn(a2, b, s, v)
            ...
            <8>2. m.bal >= b  BY <8>1, <5>1, <5>3 DEF TypeOK, AccInv, Messages
            ...
            <8>4. CASE ~ \E d \in m.decrees: d.slot = s  BY <8>4, <8>1, <5>1, <5>3, r \in aVoted'[a2] DEF TypeOK
            <8> QED BY <8>3, <8>4
      ...
      <5> QED BY <5>2, <5>3
    <4>. QED BY <4>1, <4>2, <4>3, <4>4, <2>1 DEF Next
  ...
  <3>3. MsgInv'
    ...
    <4>2. ASSUME NEW a \in Acceptors, Phase1b(a)  PROVE  MsgInv'
      ...
      <5>7. (\A b \in Ballots, s \in Slots, v \in Values: b \in MaxBallotInSlot(m1.voted, s)+1..m1.bal-1 =>
                ~VotedForIn(m1.from, b, s, v))'
        BY <5>5, <5>6, <5>1, Z3
      <5>8. CASE m.bal > aBal[a]
        ...
        <6>1. ((m2.type = "1b") => MsgInv1b(m2))'
          ...
          <7>2. CASE m2 \in msgs' \ msgs
            <8>1. m2 = m1  BY <5>1, <7>2, <5>8 DEF Send
            <8> QED BY <7>2, <5>8, Phase1bVotedForInv, <4>2, <5>2, <5>4, <5>7, <5>1, <3>1, <2>1,
        ...
        <6> QED BY <6>1, <6>2
      <5>9. CASE ~ (m.bal > aBal[a]) BY <5>9, Phase1bVotedForInv, <4>2, <5>2, <5>4, <5>7, <5>1,
    ...
    <4>3. ASSUME NEW p \in Proposers, Phase2a(p)  PROVE  MsgInv'
      ...
      <5>1. PICK Q \in Quorums, S \in SUBSET {m2 \in msgs: (m2.type = "1b") /\ (m2.bal = b)}:
              ...
              /\Send([type |-> "2a", bal |-> b, from |-> p, decrees |-> ProposeDecrees(UNION {m2.voted: m2 \in S})])
            BY <4>3 DEF Phase2a
      ...
      <5>5. ((m.type = "2a") => MsgInv2a(m))'
        ...
        <6>2. (\A d \in m.decrees: SafeAt(m.bal, d.slot, d.val))'
          <7>1. \A d \in [slot: FreeSlots(VS), val: Values]: SafeAt(b, d.slot, d.val)
            <8> SUFFICES ASSUME NEW d \in [slot: FreeSlots(VS), val: Values]  PROVE  SafeAt(b, d.slot, d.val)
                OBVIOUS
            <8>1. \A m2 \in S: ~\E d2 \in m2.voted: d.slot = d2.slot  BY DEF FreeSlots
            ...
            <8>4. \A v \in Values, b2 \in Ballots, a \in Q: b2 \in 0..b-1 => ~VotedForIn(a, b2, d.slot, v)
                  BY <5>1, <8>2, <8>3 DEF FreeSlots, TypeOK, Messages
            <8> QED BY <8>4, <3>1, <6>1 DEF SafeAt, NewProposals, FreeSlots, WontVoteIn, TypeOK, Messages
          <7>2. \A d \in [slot: FreeSlots(VS), val: Values]: SafeAt(b, d.slot, d.val)'  BY <7>1, SafeAtStable,
                <3>1, <2>1, <5>2 DEF NewProposals, FreeSlots, TypeOK, Messages
          <7>3. \A d \in NewProposals(VS): SafeAt(b, d.slot, d.val)'  BY <7>2, NPC DEF NewProposals
          <7>4. \A d \in PartialBmax(VS): SafeAt(b, d.slot, d.val)
            <8> SUFFICES ASSUME NEW d \in PartialBmax(VS), NEW b2 \in Ballots, b2 \in 0..(b-1)
            ...
            <8>2. \A m2 \in S: MaxBallotInSlot(m2.voted, d.slot) =< max
                  BY \A m2 \in S: m2.voted \subseteq VS, MBISSubsets DEF PartialBmax, TypeOK, Messages
            <8>3. ~\E d2 \in VS: (d2.bal > d.bal /\ d2.slot = d.slot)
                  BY \A d2 \in VS: ~(~(d2.bal =< d.bal) /\ d2.slot = d.slot) DEF PartialBmax, TypeOK, Messages
            ...
            <8>5. max = d.bal
              ...
              <9>2. CASE max < d.bal  BY MBISNoMore, <9>2 DEF PartialBmax, TypeOK, Messages
          ...
          <7>5. PartialBmax(VS) \in SUBSET [bal: Ballots, slot: Slots, val: Values]  BY DEF PartialBmax, TypeOK, Messages
          <7>6. \A d \in PartialBmax(VS): SafeAt(b, d.slot, d.val)'  BY <7>4, SafeAtStable, <3>1, <7>5, <2>1, <5>2
          <7>7. \A d \in Bmax(VS): SafeAt(b, d.slot, d.val)'  BY <7>6, <7>5 DEF Bmax
          <7>8. \A d \in Bmax(VS) \cup NewProposals(VS): SafeAt(b, d.slot, d.val)'  BY <7>7, <7>3
          <7>9. (\A m2 \in msgs: m2.type = "2a" => \A d \in m2.decrees: SafeAt(m2.bal, d.slot, d.val))'
            ...
            <8>2. CASE m2 \in msgs' \ msgs
              <9>1. SafeAt(m2.bal, d.slot, d.val)'  BY <7>8, <8>2, <5>1, <5>2 DEF Send, ProposeDecrees
        ...
        <6>3. (\A d1,d2 \in m.decrees: d1.slot = d2.slot => d1 = d2)'
          <7>1. \A m2 \in msgs' \ msgs: \A d1,d2 \in m2.decrees: d1.slot = d2.slot => d1 = d2
            ...
            <8>2. \A r1, r2 \in PartialBmax(VS): r1.slot = r2.slot => r1.bal = r2.bal  BY <8>1 DEF PartialBmax
            <8>3. PartialBmax(VS) \subseteq VS  BY <8>1 DEF PartialBmax
            <8>4. \A r1, r2 \in PartialBmax(VS): r1.bal = r2.bal /\ r1.slot = r2.slot => r1.val = r2.val
                  BY <8>3, VotedUnion
            <8>5. \A r1, r2 \in PartialBmax(VS): r1.slot = r2.slot => r1.bal = r2.bal /\ r1.val = r2.val
                  BY <8>4, <8>2, <8>3, <8>1
            <8>6. \A r1, r2 \in Bmax(VS): r1.slot = r2.slot => r1 = r2  BY <8>5 DEF Bmax
            <8> QED BY <8>6, NPC, <5>1 DEF ProposeDecrees, Send
    ...
    <4>4. ASSUME NEW a \in Acceptors, Phase2b(a)  PROVE  MsgInv'
      ...
      <5>2. ((m.type = "1b") => MsgInv1b(m))'
        ...
        <6>2. (\A b \in Ballots, s \in Slots, v \in Values: b \in MaxBallotInSlot(m.voted, s)+1..m.bal-1 =>
          ...
          <7>2. CASE m.from # a  BY <5>1, <3>1, <7>2, <7>1 DEF VotedForIn,
      ...
      <5>4. ((m.type = "2b") => MsgInv2b(m))'
        <6>1. CASE m \in msgs  BY <5>1, <3>1, <6>1 DEF TypeOK, Messages, Send, MsgInv, MsgInv2b
        <6>2. CASE m \in msgs' \ msgs  BY <5>1, <3>1, <6>2 DEF TypeOK, Send, MsgInv2b
        <6> QED BY <6>1, <6>2
