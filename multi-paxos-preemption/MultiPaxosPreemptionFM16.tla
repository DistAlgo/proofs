------------------------------- MODULE MultiPaxosPreemptionFM16 -------------------------------
(***************************************************************************)
...
VARIABLES msgs,             \* The set of messages that have been sent.
          ...
          acceptorMaxBal,   \* For each acceptor, the highest ballot seen
                            \* by it.
          proposerBallot    \* Current Ballots being run by proposers

vars == <<msgs, acceptorVoted, acceptorMaxBal, proposerBallot>>
...
Init == /\ msgs = {}
        ...
        /\ proposerBallot = [p \in Proposers |-> 0]
...
(* Quorum is needed.                                                       *)
(***************************************************************************)
Phase1a(p) == /\ ~ \E m \in msgs : (m.type = "1a") /\ (m.bal = proposerBallot[p])
              /\ Send([type |-> "1a", bal |-> proposerBallot[p], from |-> p])
              /\ UNCHANGED <<acceptorVoted, acceptorMaxBal, proposerBallot>>
...
Phase1b(a) == 
  \E m \in msgs : 
     /\ m.type = "1a"
     /\ IF m.bal > acceptorMaxBal[a] THEN
           /\ Send([type |-> "1b", bal |-> m.bal, from |-> a, voted |-> acceptorVoted[a]])
           /\ acceptorMaxBal' = [acceptorMaxBal EXCEPT ![a] = m.bal]
           /\ UNCHANGED <<acceptorVoted, proposerBallot>>
        ELSE
           /\ Send([type |-> "preempt", to |-> m.from, bal |-> acceptorMaxBal[a]])
           /\ UNCHANGED <<acceptorVoted, acceptorMaxBal, proposerBallot>>
...
Phase2a(p) ==
  /\ ~ \E m \in msgs : (m.type = "2a") /\ (m.bal = proposerBallot[p])
  /\ \E Q \in Quorums :
        \E S \in SUBSET {m \in msgs : (m.type = "1b") /\ (m.bal = proposerBallot[p])} :
           /\ \A a \in Q : \E m \in S : m.from = a
           /\ Send([type |-> "2a", bal |-> proposerBallot[p], decrees |-> ProposeDecrees(UNION {m.voted : m \in S}), from |-> p])
  /\ UNCHANGED <<acceptorMaxBal, acceptorVoted, proposerBallot>>
...
(* the proposer updates its proposerBallot, which is the next ballot it is *)
...
(***************************************************************************)
NewBallot(bb) ==
  CHOOSE b \in Ballots : /\ b > bb
                         /\ ~ \E m \in msgs : m.type = "1a" /\ m.bal = b
Preempt(p) ==
  \E m \in msgs :
    /\ m.type = "preempt"
    /\ m.to = p
    /\ m.bal > proposerBallot[p]
    /\ proposerBallot' = [proposerBallot EXCEPT ![p] = NewBallot(m.bal)]
    /\ UNCHANGED <<acceptorVoted, acceptorMaxBal, msgs>>
...
Messages ==      [type : {"1a"}, bal : Ballots, from : Proposers]
            ...
            \cup [type : {"preempt"}, bal : Ballots, to : Proposers]
...
TypeOK == /\ msgs \in SUBSET Messages
          ...
          /\ proposerBallot \in [Proposers -> Ballots]
...
<1>1. ASSUME NEW pp \in Proposers, Phase1a(pp)
      PROVE  SafeAt(v, b, s)'
...
<1>3. ASSUME NEW pp \in Proposers, Phase2a(pp)
      PROVE  SafeAt(v, b, s)'
...
<1>6. ASSUME NEW pp \in Proposers, Preempt(pp)
      PROVE  SafeAt(v, b, s)'
  BY <1>6, SMT DEF SafeAt, Preempt, VotedForIn, WontVoteIn
<1>5. QED
  BY <1>1, <1>2, <1>3, <1>4, <1>6 DEF Next
...
<1>2. Inv /\ [Next]_vars => Inv'
  ...
  <3>1. TypeOK'
    <4>1. ASSUME NEW p \in Proposers, Phase1a(p) PROVE TypeOK'
      ...
      <5>4. (\A a \in Acceptors: \A r \in acceptorVoted[a] : acceptorMaxBal[a] >= r.bal)'
        BY <4>1 DEF Phase1a, TypeOK
      <5>6. (proposerBallot \in [Proposers -> Ballots])'
        BY <4>1 DEF Phase1a, TypeOK
      <5>5. QED
        BY <5>1, <5>2, <5>3, <5>4, <5>6 DEF TypeOK
    ...
    <4>5. ASSUME NEW p \in Proposers, Preempt(p) PROVE TypeOK'
      <5>1. (msgs \in SUBSET Messages)'
        BY <4>5, PTL DEF Preempt, TypeOK
      <5>2. (acceptorVoted \in [Acceptors -> SUBSET [bal : Ballots, slot : Slots, val : Values]])'
        BY <4>5 DEF Preempt, TypeOK
      <5>3. (acceptorMaxBal \in  [Acceptors -> Ballots \cup {-1}])'
        BY <4>5 DEF Preempt, TypeOK
      <5>4. (\A a_1 \in Acceptors: \A r \in acceptorVoted[a_1] : acceptorMaxBal[a_1] >= r.bal)'
        BY <4>5 DEF Preempt, TypeOK
      <5>6. (proposerBallot \in [Proposers -> Ballots])'
        BY <4>5, PTL DEF Preempt, TypeOK, NewBallot, Messages
      <5>5. QED
        BY <5>1, <5>2, <5>3, <5>4, <5>6 DEF TypeOK
    <4>. QED
      BY <4>1, <4>2, <4>3, <4>4, <2>1, <4>5 DEF Next

  <3>2. AccInv'
    <4>1. ASSUME NEW p \in Proposers, Phase1a(p) PROVE AccInv'
    ...
    <4>4. ASSUME NEW a \in Acceptors, Phase2b(a) PROVE AccInv'
      ...
      <5>. PICK m \in msgs : Phase2b(a)!(m)
          BY <4>4 DEF Phase2b
      <5>1 CASE (a_1 = a /\ ~ (m.bal >= acceptorMaxBal[a])) \/ a_1 # a
      ...
      <5>2 CASE a_1 = a /\ (m.bal >= acceptorMaxBal[a])
        ...
        <6>5. QED
          BY <6>1, <6>2, <6>3, <6>4
      <5> QED BY <5>1, <5>2
    <4>5. ASSUME NEW p \in Proposers, Preempt(p) PROVE AccInv'
      <5>1. \A aa \in Acceptors, ss \in Slots :
                MaxVotedBallotInSlot(acceptorVoted[aa], ss) = MaxVotedBallotInSlot(acceptorVoted[aa], ss)'
        BY <3>1, <4>5 DEF Preempt, MaxVotedBallotInSlot
      <5> QED BY <4>5, <3>1, PreemptVotedForInv, <5>1, SMTT(100) DEF AccInv, TypeOK, Preempt, Send
    <4>. QED
      BY <4>1, <4>2, <4>3, <4>4, <4>5, <2>1 DEF Next
  ...
  <3>3. MsgInv'
      ...
      <5> SUFFICES ASSUME NEW m \in msgs'
                   PROVE  (/\ (m.type = "1b") =>
                           ...
                           /\ (m.type = "2b") => 
                                ...
                                /\ m.bal =< acceptorMaxBal[m.from])'
        BY DEF MsgInv
      <5> DEFINE b == proposerBallot[p]
      <5> PICK Q \in Quorums, S \in SUBSET {mm \in msgs : (mm.type = "1b") /\ (mm.bal = b)} :
                        /\ \A a \in Q : \E mm \in S : mm.from = a
                        /\ Send([type |-> "2a", bal |-> b, decrees |-> ProposeDecrees(UNION {mm.voted : mm \in S}), from |-> p])
          BY <4>3 DEF Phase2a
      <5>30. b = proposerBallot'[p]
        BY <4>3 DEF Phase2a
      <5>31. b \in Ballots
        BY DEF TypeOK
      ...
      <5>2. ((m.type = "2a") => 
        ...
        <6>1. (\A d \in m.decrees : SafeAt(d.val, m.bal, d.slot))'
          ...
          <7>77. \A d \in [slot : FreeSlots(VS), val : Values] \ {} : SafeAt(d.val, b, d.slot)'
            BY <7>13, SafeAtStable, <3>1, <2>1, <5>30 DEF NewProposals, FreeSlots, TypeOK, Messages
          ...
          <7>16. \A d \in PartialBmax(VS) : SafeAt(d.val, b, d.slot)'
            BY <7>15, SafeAtStable, <3>1, <7>17, <2>1, <5>30, <5>31
          ...
          <7>20. (\A mm \in msgs: mm.type = "2a" => \A d \in mm.decrees : SafeAt(d.val, mm.bal, d.slot))'
            ...
            <8>2. CASE mm \in msgs' \ msgs
              ...
              <9>3. SafeAt(d.val, mm.bal, d.slot)'
                BY <9>1, <7>19, <8>2, <5>30, <5>31 DEF Send, ProposeDecrees
              <9> QED
    ...
    <4>4. ASSUME NEW a \in Acceptors, Phase2b(a) PROVE  MsgInv'
      ...
      <5>4. QED
        BY <5>1, <5>2, <5>3
    <4>5. ASSUME NEW p \in Proposers, Preempt(p) PROVE  MsgInv'
      BY <4>5, PreemptVotedForInv, <3>1, SafeAtStable, <2>1 DEF Preempt, MsgInv, TypeOK, Messages
    <4>. QED
      BY <4>1, <4>2, <4>3, <4>4, <4>5, <2>1 DEF Next
  <3>4. QED
